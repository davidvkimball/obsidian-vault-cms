...
- Prefer Obsidian's API over Node `fs` for vault access where possible. Vault CMS does reach into the user's Astro project on disk (outside the vault) for configuration sync and route detection — that's intentional and uses Node `fs` directly.
- This plugin makes no network requests.

## Public API for other plugins

Vault CMS exposes a typed API for other Obsidian plugins to consume. See `src/api/public/vault-cms.d.ts` for the full surface — that file is the canonical reference and is meant to be copied into downstream plugin projects for type-safe access. See the README's "Public API" section for usage.
